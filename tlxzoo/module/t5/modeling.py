...
class T5DenseReluDense(Module):
    def __init__(self, initializer_factor, d_model, d_ff, dropout_rate, name="DenseReluDense", **kwargs):
        super().__init__(name=name, **kwargs)
        ...
        self.d_ff = d_ff
        self.d_model = d_model
        self.wi = tlx.nn.layers.Linear(
            d_ff, in_features=d_model, b_init=None, name=name + "/wi", W_init=wi_initializer
        )  # Update init weights as in flax
        self.wo = tlx.nn.layers.Linear(
            d_model, in_features=d_ff, b_init=None, name=name + "/wo", W_init=wo_initializer
        )  # Update init weights as in flax
        self.dropout = tlx.nn.layers.Dropout(dropout_rate, name=name + "/dropout")
        self.act = tlx.relu
...
class T5GatedGeluDense(Module):
    def __init__(self, initializer_factor, d_model, d_ff, dropout_rate, name="GatedGeluDense", **kwargs):
        super().__init__(name=name, **kwargs)
        wi_initializer = tlx.initializers.RandomNormal(
            mean=0, stddev=initializer_factor * (d_model ** -0.5)
        )
        ...
        self.wi_0 = tlx.nn.layers.Linear(
            d_ff, in_features=d_model, b_init=None, name=name + "/wi_0", W_init=wi_initializer
        )  # Update init weights as in flax
        self.wi_1 = tlx.nn.layers.Linear(
            d_ff, in_features=d_model, b_init=None, name=name + "/wi_1", W_init=wi_initializer
        )  # Update init weights as in flax
        self.wo = tlx.nn.layers.Linear(
            d_model, in_features=d_ff, b_init=None, name=name + "/wo", W_init=wo_initializer
        )  # Update init weights as in flax
...
class T5LayerFF(Module):
    def __init__(self, initializer_factor, d_model, d_ff, dropout_rate, feed_forward_proj, layer_norm_epsilon,
                 name="layer_", **kwargs):
        super().__init__(name=name, **kwargs)
        if feed_forward_proj == "relu":
            self.DenseReluDense = T5DenseReluDense(initializer_factor, d_model, d_ff, dropout_rate,
                                                   name=name + "/DenseReluDense")
        elif feed_forward_proj == "gated-gelu":
            self.DenseReluDense = T5GatedGeluDense(initializer_factor, d_model, d_ff, dropout_rate,
                                                   name=name + "/DenseReluDense")
        else:
            raise ValueError(
                f"{self.feed_forward_proj} is not supported. Choose between `relu` and `gated-gelu`"
            )
        self.layer_norm = T5LayerNorm(d_model=d_model, epsilon=layer_norm_epsilon, name=name + "/layer_norm")
        self.dropout = tlx.nn.layers.Dropout(dropout_rate, name=name + "/dropout")
        self.dropout_rate = dropout_rate
        self.d_model = d_model
        self.d_ff = d_ff
...
class T5Attention(Module):
    ...
    def __init__(self, is_decoder, use_cache, relative_attention_num_buckets, d_model, d_kv, num_heads,
                 initializer_factor, dropout_rate, has_relative_attention_bias=False, name="SelfAttention", **kwargs):
        super().__init__(name=name, **kwargs)
        ...
        self.q = tlx.nn.layers.Linear(
            self.inner_dim, in_features=self.inner_dim, b_init=None, name=name + "/q", W_init=q_initializer
        )  # Update init weights as in flax
        self.k = tlx.nn.layers.Linear(
            self.inner_dim, in_features=self.inner_dim, b_init=None, name=name + "/k", W_init=k_initializer
        )  # Update init weights as in flax
        self.v = tlx.nn.layers.Linear(
            self.inner_dim, in_features=self.inner_dim, b_init=None, name=name + "/v", W_init=v_initializer
        )  # Update init weights as in flax
        self.o = tlx.nn.layers.Linear(
            self.d_model, in_features=self.d_model, b_init=None, name=name + "/o", W_init=o_initializer
        )  # Update init weights as in flax
        self.dropout = tlx.nn.layers.Dropout(dropout_rate, name=name + "/dropout")
...
class T5LayerSelfAttention(Module):
    def __init__(self, is_decoder, use_cache, relative_attention_num_buckets, d_model, d_kv, num_heads,
                 initializer_factor, dropout_rate, layer_norm_epsilon, has_relative_attention_bias=False,
                 name="layer_", **kwargs):
        super().__init__(name=name, **kwargs)
        self.SelfAttention = T5Attention(
            is_decoder, use_cache, relative_attention_num_buckets, d_model, d_kv, num_heads,
            initializer_factor, dropout_rate,
...
class T5LayerCrossAttention(Module):
    def __init__(self, is_decoder, use_cache, relative_attention_num_buckets, d_model, d_kv, num_heads,
                 initializer_factor, dropout_rate, layer_norm_epsilon, name="layer_", **kwargs):
        super().__init__(name=name, **kwargs)
        ...
        self.layer_norm = T5LayerNorm(d_model=d_model, epsilon=layer_norm_epsilon,
...
class T5Block(Module):
    def __init__(self, is_decoder, use_cache, relative_attention_num_buckets, d_model, d_kv, num_heads,
                 ...
                 has_relative_attention_bias=False, name="block_", **kwargs):
        super().__init__(name=name, **kwargs)
        self.is_decoder = is_decoder

        self.self_attn = T5LayerSelfAttention(
            is_decoder, use_cache, relative_attention_num_buckets, d_model, d_kv, num_heads,
            initializer_factor, dropout_rate, layer_norm_epsilon,
            has_relative_attention_bias=has_relative_attention_bias,
            name=name + "/layer_._0",
        )
        ...
        self.ffn = T5LayerFF(initializer_factor, d_model, d_ff, dropout_rate, feed_forward_proj, layer_norm_epsilon,
                             name=ffn_name)
...
class T5MainLayer(Module):
    ...
    def __init__(self, is_decoder, use_cache, relative_attention_num_buckets, d_model, d_kv, num_heads,
                 initializer_factor, dropout_rate, layer_norm_epsilon, d_ff, feed_forward_proj, num_layers,
                 embed_tokens=None, name="main_layer", **kwargs):
        super().__init__(name=name, **kwargs)
        ...
        self.block = ModuleList([
            T5Block(is_decoder, use_cache, relative_attention_num_buckets, d_model, d_kv, num_heads,
                    initializer_factor, dropout_rate, layer_norm_epsilon, d_ff, feed_forward_proj,
                    has_relative_attention_bias=bool(i == 0), name=name + f"/block_._{i}")
            for i in range(num_layers)
        ])
        self.final_layer_norm = T5LayerNorm(d_model=d_model,
                                            epsilon=layer_norm_epsilon, name=name + "/final_layer_norm")
        self.dropout = tlx.nn.layers.Dropout(dropout_rate, name=name + "/dropout")
