...
class ResNet50(nn.Module):
    def __init__(self, input_shape, preact=False, use_bias=True, use_preprocess=True, include_top=False,
                 pooling=None, classes=1000, name="resnet50"):
        ...
        if include_top:
            ...
            self.include_top_dense = tlx.nn.Linear(out_features=classes, act=tlx.softmax, name='predictions')
        else:
            if pooling == 'avg':
                self.pool = tlx.nn.GlobalMeanPool2d(name='avg_pool')
            elif pooling == 'max':
    ...
    def build(self, inputs_shape):
        ...
        _, _, extra_return_tensors = self(ones)
...
def load_h5_weight(model, weight_path):
        import h5py
        ...
        for w in model.all_weights:
            ...
            head = name.split("/")
            name = head[0] + "/" + name
            name = name.replace("conv/filters:0", "conv/kernel:0")
            name = name.replace("biases:0", "bias:0")
            name = name.replace("weights:0", "kernel:0")
            name = name.replace("moving_var:0", "moving_variance:0")
            if name not in file:
                print(f"{name} do not init.")
                continue
            names.append(name)
            w.assign(file[name])
