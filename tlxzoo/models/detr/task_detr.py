...
from .config_detr import DetrForObjectDetectionTaskConfig, DetrForSegmentationTaskConfig
from .detr import DetrModel
from .modeling import *
from .feature_detr import center_to_corners_format
from scipy.optimize import linear_sum_assignment
from .utils import *


@Registers.tasks.register
class DetrForObjectDetection(BaseForObjectDetection):
    config_class = DetrForObjectDetectionTaskConfig

    def __init__(self, config=None, model=None, **kwargs):
        if config is None:
            config = self.config_class(**kwargs)

        super(DetrForObjectDetection, self).__init__(config)

        if model is not None:
            self.detr = model
        else:
            self.detr = DetrModel(self.config.model_config)

        self.model_dim = config.model_config.model_dim

        self.class_embed = Linear(input_dim=config.model_config.model_dim, output_dim=config.num_classes,
                                  name='class_embed')

        self.bbox_embed_linear1 = Linear(config.model_config.model_dim,
                                         config.model_config.model_dim, name='bbox_embed_linear1')
        self.bbox_embed_linear2 = Linear(config.model_config.model_dim,
                                         config.model_config.model_dim, name='bbox_embed_linear2')
        self.bbox_embed_linear3 = Linear(config.model_config.model_dim, 4, name='bbox_embed_linear3')
        ...
        if format == "ckpt":
            ...
            for w in self.all_weights:
                ...
                if name.endswith("biases"):
                    name = name[:-6] + "bias"
                array = tf.train.load_variable(file_path, name + "/.ATTRIBUTES/VARIABLE_VALUE")
    ...
    def forward(self, inputs, pixel_mask=None):
        ...
        transformer_output, memory, feature_maps, masks, projected_feature_map = self.detr(inputs, pixel_mask)
        ...
        output["aux"] = []
        for i in range(0, self.config.model_config.num_decoder_layers - 1):
...
@Registers.tasks.register
class DetrForSegmentation(BaseForObjectDetection):
    config_class = DetrForSegmentationTaskConfig

    def __init__(self, config: DetrForSegmentationTaskConfig = None, model=None, **kwargs):
        if config is None:
            config = self.config_class(**kwargs)

        super(DetrForSegmentation, self).__init__(config)
        ...
        # segmentation head
        hidden_size, number_of_heads = config.model_config.model_dim, 8
        intermediate_channel_sizes = [256, 512, 1024, 2048]
        ...
        if format == "ckpt":
            ...
            for w in self.all_weights:
                ...
                try:
                    ...
                    print(f"{w.name} do not load weight.")
                    continue
                names.append(name + "/.ATTRIBUTES/VARIABLE_VALUE")
                w.assign(array)
    ...
    def forward(self, inputs, pixel_mask=None):
        ...
        output = self.detr(inputs, pixel_mask=pixel_mask, return_output=True)
        features = output["feature_maps"]
        batch_size, height, width, num_channels = tlx.get_tensor_shape(features[-1])

        memory = output["memory"]
        memory = tlx.transpose(memory, [1, 0, 2])
        memory = tlx.reshape(memory, [batch_size, height, width, self.config.model_config.model_dim])
        masks = output["masks"]
        masks = tlx.reshape(masks, [batch_size, height, width])

        transformer_output = output["transformer_output"][-1]
        projected_feature_map = output["projected_feature_map"]

        bbox_mask = self.bbox_attention(transformer_output, memory, mask=masks)
        seg_masks = self.mask_head(projected_feature_map, bbox_mask, [features[2], features[1], features[0]])

        pred_masks = tlx.reshape(seg_masks, [batch_size, self.detr.config.num_queries,
...
class DetrHungarianMatcher(tlx.nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        return [(tlx.convert_to_tensor(i, dtype=tlx.int64), tlx.convert_to_tensor(j, dtype=tlx.int64)) for i, j in indices]
