...
class HRNetFeatureConfig(BaseImageFeatureConfig):
    def __init__(self,
                 do_resize=True,
                 do_normalize=False,
                 size=(256, 256),
        ...
        self.sigma = sigma
        super(HRNetFeatureConfig, self).__init__(do_resize=do_resize,
                                                 do_normalize=do_normalize, mean=mean, std=std, **kwargs)


@Registers.features.register
class HRNetFeature(BaseImageFeature):
    config_class = HRNetFeatureConfig
    ...
    def __init__(
        ...
        super(HRNetFeature, self).__init__(config, **kwargs)
    ...
    def _image_and_keypoints_process(self, image, keypoints, bbox):
        ...
        if self.config.do_resize:
            resize_ratio = [self.config.size[0] / human_instance.shape[0], self.config.size[1] / human_instance.shape[1]]
            image = self.resize(human_instance, (self.config.size[0], self.config.size[1]))
            # resized_image = tf.image.resize(images=human_instance, size=[self.resize_h, self.resize_w])
            for i in range(self.config.num_of_joints):
    ...
    def _generate_target(self, keypoints_3d, keypoints_3d_exist):
        target_weight = np.ones((self.config.num_of_joints, 1), dtype=np.float32)
        target_weight[:, 0] = keypoints_3d_exist[:, 0]

        target = np.zeros((self.config.num_of_joints, self.config.heatmap_size[0], self.config.heatmap_size[1]),
                          dtype=np.float32)
        temp_size = self.config.sigma * 3
        image_size = np.array(self.config.size)
        heatmap_size = np.array(self.config.heatmap_size)
        for joint_id in range(self.config.num_of_joints):
            ...
            x0 = y0 = size // 2
            g = np.exp(- ((x - x0) ** 2 + (y - y0) ** 2) / (2 * self.config.sigma ** 2))
    ...
    def __call__(self, image, label, *args, **kwargs):
        ...
        # image_file = label["text"].split(" ")[0]
        # image_content = tf.io.read_file(filename=image_file)
