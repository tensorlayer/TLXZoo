...
class WiderDataSetDict(BaseDataSetDict):
    ...
    def load(cls, train_limit=None, config=None):
        ...
        return cls({"train": Wider(os.path.join(config.root_path, "train"),
                                   config.train_ann_path, limit=train_limit),
                    "test": Wider(os.path.join(config.root_path, "val"), config.val_ann_path)})

    def get_face_recognition_schema_dataset(self, dataset_type, config=None):
